...
from ipvul_src.utils.coloring import COLOR, COLORED_COMBOS
from ipvul_src.utils.exceptions import IPVulException, HostHandlerException
from ipvul_src.utils.request_handler import RequestHandler
from ipvul_src.utils.logger import SystemOutLogger
from ipvul_src.utils.help_utils import HelpUtilities
from ipvul_src.lib.fuzzer import URLFuzzer
from ipvul_src.lib.host import Host
from ipvul_src.lib.scanner import Scanner, NmapScan, NmapVulnersScan, VulnersScanner
from ipvul_src.lib.sub_domain import SubDomainEnumerator
from ipvul_src.lib.dns_handler import DNSHandler
from ipvul_src.lib.waf import WAF
from ipvul_src.lib.tls import TLSHandler
from ipvul_src.lib.web_app import WebApplicationScanner
...
https://github.com/dhanrajdadhich78/IPVul
-------------------------------------------------------------------
...
@click.option("--vulners-path", default=os.path.join(MY_PATH, "utils/misc/vulners.nse"),
              help="Path to the custom nmap_vulners.nse script."
                   "If not used, IPVul uses the built-in script it ships with.")
...
@click.option("-o", "--outdir", default="ipvul_scan_results",
...
def main(target,
    ...
    try:
        ...
        if tor_routing:
            logger.info("{} Testing that Tor service is up...".format(
        ...
        if tor_routing:
            try:
                ...
                logger.info("{} Validated Tor service is up. Routing traffic anonymously\n".format(
                    COLORED_COMBOS.NOTIFY))
            except IPVulException as err:
                print("{}{}{}".format(COLOR.RED, str(err), COLOR.RESET))
                exit(3)

        main_loop = asyncio.get_event_loop()

        logger.info("{}### IPVul Scan Started ###{}\n".format(
        ...
        if not skip_nmap_scan:
            if vulners_nmap_scan:
                ...
                nmap_thread = threading.Thread(
                    target=VulnersScanner.run, args=(nmap_vulners_scan,))
                # Run NmapVulners scan in the background
                nmap_thread.start()
            else:
                logger.info("\n{} Setting Nmap scan to run in the background".format(
                    COLORED_COMBOS.INFO))
                nmap_scan = NmapScan(
                    host=host,
                    port_range=port,
        ...
        if not no_url_fuzzing:
            fuzzer = URLFuzzer(host, ignored_response_codes,
                               threads, wordlist, follow_redirects)
        ...
        print("{}Keyboard Interrupt detected. Exiting{}".format(
