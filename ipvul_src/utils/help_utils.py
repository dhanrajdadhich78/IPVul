...
from ipvul_src.utils.exceptions import IPVulException, ScannerException, RequestHandlerException
...
class HelpUtilities:
    ...
    def validate_target_is_up(cls, host):
        ...
        try:
            ...
            try:
                ...
                raise IPVulException("Target {} seems to be down (no response to ping or from a web server"
    ...
    def parse_cookie_arg(cls, cookie_arg):
        try:
            ...
            raise IPVulException("Cookie parsing error occurred, probably due to invalid cookie format.\n"
    ...
    def validate_proxy_args(cls, *args):
        ...
        if not supplied_proxies:
            ...
            raise IPVulException("Must specify only one of the following:\n"
    ...
    def validate_executables(cls):
        if not (cls.find_nmap_executable() and cls.find_openssl_executable()):
            raise IPVulException("Could not find Nmap or OpenSSL "
                                   "installed. Please install them and run IPVul again.")
        if system() == "Darwin":
            if not cls.find_mac_gtimeout_executable():
                raise IPVulException("To support IPVul with macOS 'gtimeout' must be installed.\n"
    ...
    def confirm_traffic_routs_through_tor(cls):
        ...
        try:
            ...
            if "Congratulations. This browser is configured to use Tor." in page.text:
                ...
                raise IPVulException("Traffic does not seem to be routed through Tor.\nExiting")
        except RequestHandlerException:
            raise IPVulException("Tor service seems to be down - not able to connect to 127.0.0.1:9050.\nExiting")
